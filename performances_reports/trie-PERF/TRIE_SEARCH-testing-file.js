...
const recipes = [
        {
            ...
            "time": 10,
            "description": "Mettre les glaçons à votre goût dans le blender, ajouter le lait, la crème de coco, le jus de 2 citrons et le sucre. Mixer jusqu'à obtenir la consistence désirée",
            "appliance": "Blender",
            "ustensils": [
                "cuillère à Soupe",
                "verres",
        ...
        },
        {
            ...
            "time": 60,
            "description": "Découper le thon en dés, mettre dans un plat et recouvrir de jus de citron vert (il mieux vaut prendre un plat large et peu profond). Laisser reposer au réfrigérateur au moins 2 heures. (Si possible faites-le le soir pour le lendemain. Après avoir laissé mariner le poisson, coupez le concombre en fines rondelles sans la peau et les tomates en prenant soin de retirer les pépins. Rayer la carotte. Ajouter les légumes au poisson avec le citron cette fois ci dans un Saladier. Ajouter le lait de coco. Pour ajouter un peu plus de saveur vous pouver ajouter 1 à 2 cuillères à soupe de Crème de coco",
            "appliance": "Saladier",
            "ustensils": [
                "presse citron"
            ]
        ...
        },
        {
            ...
            "time": 50,
            "description": "Faire cuire le riz. Une fois le riz cuit, le laisser refroidir. Couper les oeufs dur en quarts ou en lamelles au choix, coupez les tomates en dés, ajouter au riz les oeufs, les tomates, le poisson, le maïs et la vinaigrette. Ajouter au gout de chacun des cornichonss, olives etc..",
        ...
        },
        {
            ...
            "time": 15,
            "description": "Peler le concombre et le couper 2, retirer les pépins. Couper les olives en morceaux, ainsi que le fromage de chèvre. Ajouter le basilic ainsi que le vinaigre balsamique et l'huile d'olive à votre gout.",
            "appliance": "Saladier",
            "ustensils": [
                "cuillère en bois",
                "couteau"
        ...
        },
        {
            ...
            "time": 40,
            "description": "Cuire les pommes de terre environ 30 minutes. Découper les échalottes finement. Durant la cuisson des pommes de terre. Préparez la vinaigrette avec l'huile d'olive et le vinaigre de cidre. Salez poivrez à discrétion. Dans un saladier, mettre la mâche.",
            "appliance": "Casserole",
            "ustensils": [
                "couteau",
                "saladier",
        ...
        },
        {
            ...
            "time": 45,
            "description": "Faire cuire les pâtes si vous n'avez pas de pennes des coquillettes peuvent faire l'affaire. Découper les tomates en petits morceaux, soit en tranches soit en dés. Coupez le basilic en petites morceaux et mélangez le aux tomates. Coupez la mozzarella en tranches. Préchauffez le four à 200°. Alternez entre couches de pates et couches de tomates, terminez par une couche de pates et recouvrir du fromage. Laisser au four 30 minutes et régalez vous ! Une recette simple qui fera plaisir au petits comme aux grands.",
            "appliance": "Four",
            "ustensils": [
                "plat à gratin",
                "couteau",
        ...
        },
        {
            ...
            "time": 15,
            "description": "Coupez les fraises en morceaux, découpez la chair de la pastèque en retirant les pépins. Mettre le tout dans le blender. Ajouter une cuillère à soupe de jus de citron ainsi que les glaçons. Ajoutez quelques feuilles de menthe pour plus de fraicheur. Mixez le tout. Servir et déguster.",
            "appliance": "Blender",
            "ustensils": [
                "verres",
                "couteau",
...
class Trie {
    constructor(){
        ...
        this.add = function(input, node = this.root) {
            ...
            if ( input.length === 0 ) {
                node.setEndOfAword();
                let fullWord = this.getCurrentWord();

                if ( !node.parentRecipeObjects.has(fullWord)){ 

                    let arrOfRecipes = [];
                    node.parentRecipeObjects.set(fullWord,arrOfRecipes);

                    let currentRecipe = this.getCurrentRecipeObject();
                    arrOfRecipes.push(currentRecipe);

                } else if (node.parentRecipeObjects.has(fullWord)){ // (check anyway)

                    let folderOfRecipesKey = node.parentRecipeObjects.get(fullWord);
                    let currentRecipe = this.getCurrentRecipeObject();
                    folderOfRecipesKey.push(currentRecipe);
                }
                return;

            } else if (!node.keys.has(input[0])) { node.keys.set(input[0], new Node());return this.add(input.substring(1), node.keys.get(input[0]));
        ...
        this.contains = function(word) {
            ...
            return ( node.keys.has(word) && node.keys.get(word).isEndOfAword());
        };
        ...
        this.searchElementInTrie = function(searchterm) {
            ...
            for ( let i=0; i < arrFromSearchterm.length; i++ ) {
                ...
                if (node.keys.has(currentLetterSearching)) {
                    ...
                    currentlyFound += currentLetterSearching;

                    if ( i >= 2 ) { 
                        if (node.parentRecipeObjects.size > 0) { completeWords.push(node.parentRecipeObjects); }
...
function processIfSeveralWords(str){
    ...
    if ( isSeveralWordsRegex.test(str) ) {
        ...
        let remove2charsWords = str.replace(isA2charsWord,'');
        let arrFromStr = remove2charsWords.split(isSeveralWordsRegex);
        return arrFromStr;
    } else {
...
function removePunctuation(str) {
    ...
    return str = punctuationless.replace(/\s{2,}/g,' ');
}


function mapDataToTree(recipes) {
    ...
    recipes.forEach(recipe => {
        ...
        let recipeNameWords = processIfSeveralWords(recipeName);
        recipesTrie.setCurrentWord(recipeName);
        recipesTrie.setCurrentRecipeObject(recipe);
        
        recipeNameWords.forEach( word => { recipesTrie.add(word, node);
        });
        

        let recipeIngredients = recipe.ingredients;
        recipeIngredients.forEach(item => {
            let ingredientName = item.ingredient;
            ingredientName = checkString(ingredientName);           
            let ingredientNameWords = processIfSeveralWords(ingredientName);
            recipesTrie.setCurrentWord(ingredientName);
            recipesTrie.setCurrentRecipeObject(recipe);
            ingredientNameWords.forEach(word => {recipesTrie.add(word, node);
            });
        });
        ...
        recipesTrie.setCurrentRecipeObject(recipe);
        recipeApplianceWords.forEach(word => { recipesTrie.add(word, node);
        });
        ...
        recipeUstensils.forEach( ustensil => {
            ...
            recipesTrie.setCurrentRecipeObject(recipe);
            ustensilWords.forEach( word => {  recipesTrie.add(word, node);
            });
        });
    });
...
let currentSearchTerm = 'chocolat';
recipesTrie.searchElementInTrie(currentSearchTerm);
